//Files and colors that correspond to each file's data
var filepaths = ["/data/seahawks.json", "/data/broncos.json"];
var colors = d3.scale.category10();

//First loads the 2 data files
var q = queue(2)
	.defer(d3.json, filepaths[0])
	.defer(d3.json, filepaths[1]);
...
q.awaitAll(function(error, results) {
	...
	for (var i = 0; i < results.length; ++i) {
		...
		//Assign class to identify data
		var fileNameStart = filePath.lastIndexOf("/");
		var lastInd = filePath.lastIndexOf(".");
		var className = filePath.substring(fileNameStart + 1, lastInd);
